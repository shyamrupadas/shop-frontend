...
export const incrementProductInCartThunk = createAsyncThunk<Cart, ProductId, { state: RootState }>(
...
function makeProductCountChange(productId: ProductId, getState: () => RootState) {
    return async (count: number = 1) => {
        const userId = getState().user.userInfo?._id; // TODO: Связанность с user-ом

        if (!userId) {
            throw new Error('Для добавления товара в корзину необходимо авторизоваться');
        }
        ...
        if (productsMap.has(productId)) {
            ...
            product && productsMap.set(productId, {
                ...product,
        ...
        // Дабавляем новый товар
            ...
            const newCartProduct: CartProductUpdate = { product: productId, count: 1 };
            productsMap.set(productId, newCartProduct);
        }

        try {
            const updatedCart = await cartApi.updateCartByUserId(userId, [...productsMap.values()]);
